...
uint16_t correnteDesligamento = 893;
uint16_t sobrecorrenteLimite = 1190;
uint8_t ctrl = 4;
//------------------------IHM------------------------------//
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	...
	if(adc_voltCalcS >= correnteDesligamento || ctrl == EMERGENCIA || (adc_voltCalcS < 15 && ctrl == LIGADO)){
	...
	if(adc_voltCalcS >= sobrecorrenteLimite){
		return;
	}
	/*if(adc_voltCalcS < 15 && ctrl == LIGADO){
		HAL_TIM_OC_Stop_IT(&htim3, TIM_CHANNEL_1);
		dummy1 = ANGULO_TOTAL_DE_VARIACAO - 1;
		pulso[0] = 20 + dummy1;
		pulso[1] = 28 + dummy1;
...
void HAL_ADC_ConvCpltCallback(ADC_HandleTypeDef* hadc){
	adc_volt[ctrlADC] =  exp(converteBitVolt(HAL_ADC_GetValue(&hadc1)));
	ctrlADC++;
	...
	if(ctrlADC == 120){
		for(uint8_t i=0;i!=120;i++){
		 adc_voltCalc = (adc_voltCalc + adc_volt[i]);
			 if(i == 119){
				 adc_voltCalcS = sqrt(adc_voltCalc/120);
				 adc_voltCalc = 0;
			 }
		}
		sprintf(msg2, "%.2fmA\n", adc_voltCalcS);
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart){
	...
	if(strstr(msg, "cdm") != NULL){
		for(int j = 0; j!=3;j++){
			msg[j] = '0';
		}
		correnteDesligamento = atoi(msg);
		limpaBuffer(msg);
	}
	if(strstr(msg, "cdc") != NULL){
		for(int j = 0; j!=3;j++){
			msg[j] = '0';
		}
		correnteDesligamento = atoi(msg)/10;
		limpaBuffer(msg);
	}
	if(strstr(msg, "scc") != NULL){
		for(int j = 0; j!=3;j++){
			msg[j] = '0';
		}
		sobrecorrenteLimite = atoi(msg)/10;
		limpaBuffer(msg);
	}
	HAL_UART_Transmit(&huart2, (uint8_t*)msg, strlen(msg), TIMEOUT);
	HAL_UART_Receive_IT(&huart2, (uint8_t*)msg, 7);
